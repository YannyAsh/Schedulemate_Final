...
<div class="container-fluid px-4">
    <div class="row g-3 my-2">
                ...
                <div class="table-responsive">
                    <table id="myTable" class="table table-stripped">
                        ...
                        <tbody>
                            ...
                            $conditions = ['roomCollege' => $college, 'status' => 0];
                            $sql = $db->getAllRowsFromTableWhere('tb_room', $conditions);
                            $i = 1;
                            // Display Active Rooms
                            foreach ($sql as $row) {
                                $showButtons = $row["roomProgram"] == $program;
                            ?>
                                <tr>
                                    <td><?php echo $i; ?></td>
                                    ...
                                    <td>
                                        <?php if ($showButtons) { ?>
                                            <a href="" name="room_edit" class="edit" data-id="<?php echo $row["roomID"] ?>" data-bs-toggle="modal">
                                                <i class="material-icons" data-bs-toggle="tooltip" title="Edit">&#xe254;</i>
                                            </a>
                                            <input type="hidden" name="roomID" value="<?php echo $row['roomID']; ?>">
                                            <a href="#statusRoom" class="status" data-bs-toggle="modal" data-roomid="<?php echo $row['roomID']; ?>">
                            ...
                            // Fetch inactive rooms with the same program and college
                            $conditions = ['roomCollege' => $college, 'status' => 1];
                            $sql2 = $db->getAllRowsFromTableWhere('tb_room', $conditions);
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="text-danger">
                                        <?php if ($showButtons) { ?>
                                            <a href="#statusRoomActivate" class="status" data-bs-toggle="modal" data-roomid="<?php echo $row['roomID']; ?>">
                                                <i class="material-icons" data-bs-toggle="tooltip" title="Status">&#xe86c;</i>
                                            </a>
                                        <?php } ?>
                                    </td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="POST" action="room_all_process.php">
                        ...
                        <input type="hidden" name="roomStatus" value="0"> <!-- Always set to "0" for active status -->
        ...
        <div id="statusRoomActivate" class="modal fade">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="POST" action="room_all_process.php">
                        <input type="hidden" name="roomID" id="roomID" value="<?php echo $roomID; ?>">
                        ...
                        <div class="modal-body">
                            <h6>Are you sure you want to change the Status of this Room?</h6>.
                            <input type="hidden" name="roomID" id="status_roomIDz" value="">
                        </div>

                        <div class="modal-footer">
                            <input type="button" class="btn" data-bs-dismiss="modal" value="Cancel">
                            <input type="submit" class="btn" name="room_toggle_statusActivate" value="Confirm Status">
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    $('.status').on('click', function() {
        ...
        $('#status_roomIDz').val(roomID); // Set roomID to the hidden input field
